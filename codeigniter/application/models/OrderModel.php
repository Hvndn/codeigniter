...
class OrderModel extends CI_Model
{
    ...
    {
        // 1. Lấy danh sách đơn hàng + thông tin shipping, sắp xếp theo thời gian sớm nhất
        ...
        $this->db->order_by('o.created_at', 'ASC'); // thời gian sớm nhất lên đầu
        $this->db->order_by('o.id', 'ASC');        // thứ tự thứ cấp nếu trùng created_at
        ...
        // 2. Lấy tất cả chi tiết sản phẩm của các đơn hàng này cùng lúc
        $order_codes = array_column($orders, 'order_code');

        $this->db->select('od.order_code, od.product_id, od.quantity, p.title, p.price');
